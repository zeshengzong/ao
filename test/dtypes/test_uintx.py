...
class TestUintxQuantization(TestCase):
    def tearDown(self):
        torch._dynamo.reset()
    ...
    @skipIf(not TORCH_VERSION_AT_LEAST_2_5, "only works with fix in the nightly build")
    ...
    @parametrize("dtype", dtypes)
    @skipIf(not torch.cuda.is_available(), "Need CUDA available")
    @skipIf(not TORCH_VERSION_AT_LEAST_2_3, "sub byte dtype requires torch 2.3+")
    ...
    @skipIf(
        ...
        "torch.compile without unwrap_tensor_subclass requires torch 2.5+",
    )
...
instantiate_parametrized_tests(TestUintxQuantization)
